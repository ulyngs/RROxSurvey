...
# plot per Div pgrdata_Awareness
{
...
# plot for all pgrdata_Awareness
{
All_pgrdata_Awareness <- pgrdata_Awareness[,c("LabelIndiv", "Answer", "n")] %>% group_by(LabelIndiv, Answer) %>% summarise (n = sum(n, na.rm=TRUE)) 
All_pgrdata_Awareness <- All_pgrdata_Awareness  %>% group_by(LabelIndiv) %>% mutate(perc = n / sum(n) * 100 )

All_pgrdata_Awareness$LabelIndiv <- factor(All_pgrdata_Awareness$LabelIndiv, levels = Measures) # this will determine order of the bars
str(All_pgrdata_Awareness)

All_pgrdata_Awareness_plot <- ggplot(All_pgrdata_Awareness) +      
  
  ### Add the stacked bar
  geom_bar(aes(x=LabelIndiv, y=perc, fill=factor(Answer, 
                                                    level = c("Practicing myself", "Accessing / using only", "Aware only",  "Not aware / not sure if applicable",  "Not applicable" ))), 
           stat="identity", alpha=0.5) +
 
  scale_fill_manual(values = c("#ABDDA4", "#FFFFBF", "#FDAE61", "#D7191C", "black"), 
                    breaks=c("Practicing myself", "Accessing / using only", "Aware only", "Not aware / not sure if applicable", "Not applicable"))+
