...
# plot for all pgrdata_Effect
{
  All_pgrdata_Effect <- pgrdata_Effect[,c("LabelIndiv", "Answer", "n")] %>% group_by(LabelIndiv, Answer) %>% summarise (n = sum(n, na.rm=TRUE)) 
  All_pgrdata_Effect <- All_pgrdata_Effect  %>% group_by(LabelIndiv) %>% mutate(perc = n / sum(n) * 100 )
  
  All_pgrdata_Effect$LabelIndiv <- factor(All_pgrdata_Effect$LabelIndiv, levels = Measures) # this will determine order of the bars
  str(All_pgrdata_Effect)
  
  All_pgrdata_Effect_plot <- ggplot(All_pgrdata_Effect) +      
    
    ### Add the stacked bar
    geom_bar(aes(x=LabelIndiv, y=perc, fill=factor(Answer, 
                                                   c("Beneficial", "Neutral(neither detrimental nor beneficial)", "Detrimental",  "Not sure",  "Not applicable" ))), 
             stat="identity", alpha=0.5) +
    

    scale_fill_manual(values = c("#1B9E77", "#E6AB02", "#D95F02", "#666666", "black"), 
                      breaks=c("Beneficial", "Neutral(neither detrimental nor beneficial)", "Detrimental", "Not sure", "Not applicable"),
                      labels =c("Beneficial", "Neutral (neither detrimental nor beneficial)", "Detrimental", "Not sure", "Not applicable")
                      , drop = FALSE) +
